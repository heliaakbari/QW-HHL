...
parser.add_argument("--testind", type=int, required=False)
...
if args.mode == "kappatest":
    ...
    msystem.PrepSystem()
elif args.mode == "matlabinit":
    msystem.FileInitMatlab(A_file="./matrix_dat/matrix_"+str(args.dimension)+"_"+str(args.testind)+".dat", b_file="./matrix_dat/rvec_"+str(args.dimension)+"_"+str(args.testind)+".dat")
    msystem.PrepSystem()
else:
...
print("Done.")
sys.stdout.flush()
with open("./benchmarking/res_"+str(args.dimension)+"_"+str(args.testind)+"_"+str(args.phase)+".dat", "w") as f:
    with redirect_stdout(f):
        print("%20.10f"%(t1-t0))
        print("%20d"%(circ_transpiled.size()))
...
msystem.CompareClassical(sol,args)
